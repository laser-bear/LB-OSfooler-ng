...
from dpkt import *
from scapy import *
from scapy import config, supersocket
...
config.verbose = 0
config.L3socket = supersocket.L3RawSocket
...
def send_icmp_response(pl, probe):
    ...
    if (base[probe][3][1] == "Z"):
        ...
        send(old_div(IP(id=icmp_ipid, dst=inet_ntoa(pkt.src), src=inet_ntoa(pkt.dst), flags=frag_bit,
             ttl=TG), ICMP(id=pkt.icmp.data.id, seq=pkt.icmp.data.seq, code=code, type=0)), verbose=0)
...
def print_tcp_packet(pl, destination):
    ...
    if opts.verbose:
        print(" [+] Modifying '%s' packet in real time (total length %s)" %
              (destination, pl.get_payload_len()))
...
def print_icmp_packet(pl):
    pkt = ip.IP(pl.get_payload())
    if opts.verbose:
        print(" [+] Modifying packet in real time (total length %s)" %
              pl.get_payload_len())
        print("      [+] IP:   source %s destination %s tos %s id %s" %
              (inet_ntoa(pkt.src), inet_ntoa(pkt.dst), pkt.tos, pkt.id))
        print("      [+] ICMP: code %s type %s len %s id %s seq %s" % (pkt.icmp.code,
              pkt.icmp.type, len(pkt.icmp.data.data), pkt.icmp.data.id, pkt.icmp.data.seq))
...
def print_udp_packet(pl):
    ...
    if opts.verbose:
        ...
        print("      [+] IP:   source %s destination %s tos %s id %s" %
              (inet_ntoa(pkt.src), inet_ntoa(pkt.dst), pkt.tos, pkt.id))
        print("      [+] UDP:  sport %s dport %s len %s" %
              (pkt.udp.sport, pkt.udp.dport, len(pkt.udp.data)))
...
def main():
    ...
    if opts.interface:
        ...
        try:
            ...
            q_num1 = os.listdir(
                "/sys/class/net/").index(opts.interface) * 2 + 1
        except ValueError as err:
            q_num0 = -1
            q_num1 = -1
    else:
        # you may paste here your main interface found by '$~: ip a', for instance
        interface = "eth0"
        try:
            q_num0 = os.listdir("/sys/class/net/").index(opts.interface) * 2
            q_num1 = os.listdir(
    ...
    if opts.os:
        ...
        if (not base):
            print(
                "      [->] \"%s\" could not be found in nmap database..." % opts.os)
            sys.exit(' [+] Aborting...')
    ...
    if (opts.osgenre):
        ...
        if (not exists):
            print(
                "      [->] Could not found that combination in p0f database...")
            sys.exit(' [+] Aborting...')
    ...
    if (opts.osgenre):
        ...
        os.system(
            "iptables -A OUTPUT -t mangle -p TCP --syn -j NFQUEUE --queue-num %s" % q_num1)
    ...
    try:
        ...
        if (q_num0 >= 0):
            os.system(
                "iptables -D INPUT -t mangle -j NFQUEUE --queue-num %s" % q_num0)
        if (q_num1 >= 1):
            os.system(
                "iptables -D OUTPUT -t mangle -p TCP --syn -j NFQUEUE --queue-num %s" % q_num1)
